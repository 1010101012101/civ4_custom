...
bool CvCityAI::IsMaintainHigh()
{
	...
	{
		sumMain+=pLoopCity->getMaintenance();
	}
	return (getMaintenance()*2/3)>sumMain/GET_PLAYER(getOwnerINLINE()).getNumCities();
}
...
void CvCityAI::AI_chooseProduction()
{
    ...
    {
		if (!kPlayer.AI_isFinancialTroubleAddCost(NULL,kPlayer.AI_CalcNewCityCost(NULL)))
		{
			iMaxSettlers= std::min(1, iNumAreaCitySites + iNumWaterAreaCitySites);
		}
	...
	if (IsMaintainHigh())
	{
		if(AI_chooseBuilding(BUILDINGFOCUS_MAINTENANCE))
		{
			return;
		}
	}
