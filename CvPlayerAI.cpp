...
{
	m_NewCityCostCache=0;
	m_NewCityCostCityNumCache=0;
...
int CvPlayerAI::AI_CalcNewCityCost(CvCity* pCity) const
{
	...
	int curMain=0,newMain=0;
	for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
	{
		tmp=CalculateNumCitiesMaintenanceTimes100(getNumCities(),pLoopCity->getPopulation());
		tmp=tmp * std::max(0, (pLoopCity->getMaintenanceModifier() + 100));
		tmp/=100;
		curMain+=tmp;
	}
	...
	for (pLoopCity = firstCity(&iLoop); pLoopCity != NULL; pLoopCity = nextCity(&iLoop))
	{
		tmp=CalculateNumCitiesMaintenanceTimes100(getNumCities()+1,pLoopCity->getPopulation());
		tmp=tmp * std::max(0, (pLoopCity->getMaintenanceModifier() + 100));
		tmp/=100;
		newMain+=tmp;
	}
	cost=newMain-curMain;

	if (pCity==NULL)
	{
		tmp=CalculateNumCitiesMaintenanceTimes100(getNumCities()+1,3);
		tmp/=100;
		cost+=tmp;

		cost-=6; //new city quick earn;

	}
	else
	{
		tmp=CalculateNumCitiesMaintenanceTimes100(getNumCities()+1,pCity->getPopulation());
		tmp+=CalculateDistanceMaintenanceTimes100(pCity);
		tmp=tmp * std::max(0, (pCity->getMaintenanceModifier() + 100));
		tmp/=100;
		cost+=tmp;

		cost-=pCity->getYieldRate(YIELD_COMMERCE);
	}
...
void CvPlayerAI::AI_conquerCity(CvCity* pCity)
{
	...
	if (canRaze(pCity))
	{
		...
		if (getNumCities() > 2 && AI_isFinancialTroubleAddCost(NULL,AI_CalcNewCityCost(pCity)))
		{
			iRazeValue+=100;
		}
...
int CvPlayerAI::AI_cityTradeVal(CvCity* pCity) const
{
	...
	int cost=AI_CalcNewCityCost(pCity);
	if (AI_isFinancialTroubleAddCost(NULL,cost))
	{
		iValue+=-cost*20;
	}
...
DenialTypes CvPlayerAI::AI_cityTrade(CvCity* pCity, PlayerTypes ePlayer) const
{
	...
	if (!(GET_PLAYER(ePlayer).isHuman()))
	{
		if (GET_PLAYER(ePlayer).getTeam() != getTeam())
		{
			...
			{
				if (GET_PLAYER(ePlayer).AI_isFinancialTroubleAddCost(NULL,GET_PLAYER(ePlayer).AI_CalcNewCityCost(pCity)))
				{
...
// XXX
bool CvPlayerAI::AI_isFinancialTroubleAddCost(int* earnFund,int addedCost) const
{
	//if (getCommercePercent(COMMERCE_GOLD) > 50)
	{
		int iNetCommerce = 1 + getCommerceRate(COMMERCE_GOLD) + getCommerceRate(COMMERCE_RESEARCH) + std::max(0, getGoldPerTurn());
		int iNetExpenses = calculateInflatedCosts() + std::max(0, -getGoldPerTurn());
		iNetExpenses+=addedCost;
		int iFundedPercent = (100 * (iNetCommerce - iNetExpenses)) / std::max(1, iNetCommerce);
		...
		int safeturn=getGold()/iNetExpenses;
		if (safeturn>0)
		{
			iSafePercent-=safeturn;
		}
		if (earnFund)
		{
			*earnFund=iNetCommerce - iNetExpenses;
		}
		if (iFundedPercent < iSafePercent)
		{
			return true;
